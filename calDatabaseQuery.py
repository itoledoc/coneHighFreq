...
import analysisUtilsLight as aU
import socket
            ...
            for i in range(showrows):
                ...
                if (verbose or sourcenameWildcard):
                    ...
                    if (type(m['uvmin']) == NoneType or type(m['uvmax'])==NoneType):
                        ...
                        if (uvmin > 1e10):
                            uvmin = 0
                        print '%5d |  %7.3f +- %4s  | %10s | %6.2f GHz | %9s | %14s | %.0f-%.0f |%s' % (i+1, m['flux'],
                                      m['flux_uncertainty'], dateString,
                                      m['frequency']/1e9, catalog_id, src, uvmin, m['uvmax'],polarization)
...
####################
    ...
    def readVLBICatalog(self, catalog='/users/dbarkats/vlbi_compare/rfc_2013d_cat.txt'):
        f=open(catalog,'r')
        lines = f.readlines()
        f.close()
...
####################
    ...
    def makeSinglePlot(self,realName, all_m, fignum = 0):
        ...
        yl = ylim()
        self.makeXgridDates()
        vlines([self.y11,self.y12, self.y13,self.y14, self.y15, self.y16],0,1.1*yl[1],colors='k', linestyles='dashed')
        ...
        xlim([55550,57400]);ylim([0,1.1*yl[1]])
        #ax.yaxis.grid(T)
...
####################
                ...
                for j in range(size(measurements2)):
                    ...
                    if ra2 == ra1 or dec2 == dec1:
                        ...
                        if angle > 0.01:
                            ...
                            print "Ra/Dec1 : ",self.decimaldeg2dms(ra1,raflag=True), self.decimaldeg2dms(dec1,raflag=False)
                            print "Ra/Dec2 : ",self.decimaldeg2dms(ra2,raflag=True), self.decimaldeg2dms(dec2,raflag=False)
    ...
    def findNearestVLBISources(self, radecString, radius=3, catalog='rfc_2014b_cat.txt', 
        ...
        if (not os.path.exists(catalog)):
            ...
            os.system(cmd)
        sname,stype,s_ra,s_dec,s_radeg,s_decdeg = self.readVLBICatalog(catalog)
    ...
    def __checkOfficialName(self):
        ...
        for sourceId in range(14000):
            a = self.s.sourcecat.getSourcePlus(sourceId,False)
    ...
    def __fixCratesRADEC(self, measurements):
        """
